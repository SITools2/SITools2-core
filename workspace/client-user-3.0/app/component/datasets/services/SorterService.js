 ...
 * ImageChooser, loadUrl, extColModelToStorage, SitoolsDesk, sql2ext
 */

Ext.namespace('sitools.user.component.datasets.services.SorterService');
...
Ext.define('sitools.user.component.datasets.services.SorterService', {
    ...
    onValidate : function () {
        ...
        var sorters = [];
        Ext.iterate(this.f.getValues(), function (key, value) {
            var index = key.substring(key.indexOf('_') + 1);
            if (!Ext.isEmpty(value)) {
                if (Ext.isEmpty(sorters[index])) {
                    sorters[index] = {};
                }
                if (key.indexOf('field') != -1) {
                    sorters[index].property = value;
                } else {
                    sorters[index].direction = value;
                }
            }
        });
        ...
        if (sorters.length < 1) {
            ...
            this.store.sort(sorters[0].property, sorters[0].direction);
        } else {
    ...
    /**
     * Called on add button : Adds a new sort option.
     */
    onAdd : function () {
    ...
    /**
     * Called when remove button is pressed. Remove the last sort option.
     */
    onRemove : function () {
...
sitools.user.component.datasets.services.SorterService.executeAsService = function (config) {
    var sorterTool = Ext.create("sitools.user.component.datasets.services.SorterService", config);
    sorterTool.show();
};
