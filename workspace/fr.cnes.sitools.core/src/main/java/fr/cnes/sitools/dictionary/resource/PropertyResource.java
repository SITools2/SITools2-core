...
public final class PropertyResource extends AbstractConceptTemplateResource {
    ...
    public void doInit() {
        ...
        propertyId = getAttribute("propertyId");
    }
     ...
     */
    @Get
    public Property getRepresentation() throws ResourceException {
        try {
            ConceptTemplate template = getStore().retrieve(templateId);
            Property propertyFound = null;

            if (template == null) {
                throw new ResourceException(Status.CLIENT_ERROR_NOT_FOUND, "TEMPLATE_NOT_FOUND");
            } else {
                List<Property> properties = template.getProperties();
                for (Property property : properties) {
                    if (property.getName().equals(propertyId)) {
                ...
                if (propertyFound == null) {
                    throw new ResourceException(Status.CLIENT_ERROR_NOT_FOUND, "PROPERTY_NOT_FOUND");
                }
