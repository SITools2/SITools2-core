...
import fr.cnes.sitools.solr.AbstractSolrResource;
 ...
 * 
 */
public final class SolrDirectoryActionResource extends AbstractSolrResource {
  ...
  /**
   * Cancel the current operation on index
   * 
   * @return a {@link Response} with the cancel status
   */
  private Response cancelOperation() {
    Response response;
   ...
   * Delete a Solr Index
   * 
   * @return a {@link Response} with the delete status
   */
  ...
  private Response createIndex(Representation entity) {
    ...
    if (entity.getMediaType().isCompatible(MediaType.APPLICATION_JAVA_OBJECT)) {
      try {
        ...
        DirectoryConfigDTO dataConfig = (DirectoryConfigDTO) solR.getDataConfigDTO();
   ...
   */
  private boolean createDataConfigFile(DirectoryConfigDTO dataConfig, String solRIndexConfDir) {
    try {

      Representation dataConfigFtl = new ClientResource(LocalReference.createClapReference(getClass().getPackage())
          + "/directory-data-config.ftl").get();
