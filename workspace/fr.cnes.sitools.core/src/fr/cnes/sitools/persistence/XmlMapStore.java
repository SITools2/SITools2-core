 ...
 */
public abstract class XmlMapStore<T extends IResource> extends XmlMapStoreXStream<T> implements SitoolsStore<T> {
   ...
   * @param cl
   *          the class to keep
   * @param context org.restlet.Context
   */
  public XmlMapStore(Class<T> cl, Context context) {
    super(context);
   ...
   * @param context org.restlet.Context
   */
  public XmlMapStore(Class<T> cl, File location, Context context) {
  ...
  public T[] getArray() {
    ...
    if ((getMap() != null) && (getMap().size() > 0)) {
      ...
      result = getMap().values().toArray(array);
    }
    else {
   ...
   * @param resource T  to be created
  ...
  public T create(T resource) {
    ...
    result = getMap().get(resource.getId());
    if (result == null) {
