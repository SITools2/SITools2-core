...
import fr.cnes.sitools.feeds.representation.AtomRepresentation;
import fr.cnes.sitools.feeds.representation.RssRepresentation;
...
public final class FeedsClientResource extends SitoolsResource {
    ...
    private Representation getClassicFeed(FeedModel feedModel) {
        String selfUrl = getRequest().getResourceRef().getBaseRef().toString();
        SitoolsSettings settings = ((SitoolsApplication) getApplication()).getSettings();
        if (FeedModel.RSS_2_0.equals(feedModel.getFeedType())) {
            return new RssRepresentation(feedModel, selfUrl, settings.getPublicHostDomain());
        } else {
            return new AtomRepresentation(feedModel, selfUrl, settings.getPublicHostDomain());
        }
