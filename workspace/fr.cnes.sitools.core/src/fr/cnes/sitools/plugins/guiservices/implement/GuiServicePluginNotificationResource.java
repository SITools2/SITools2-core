 ...
 */
public final class GuiServicePluginNotificationResource extends AbstractGuiServicePluginResource {

  @Override
  public void sitoolsDescribe() {
    setName("GuiServicePluginNotificationResource");
    setDescription("Resource handling notifications on gui services plugins");
    setNegotiated(false);
  }
  ...
  public Representation notification(Representation representation, Variant variant) {
    try {
      ...
      if ((notification != null) && "DELETED".equals(notification.getStatus())) {
        ...
        // Business service
        boolean ok = getStore().delete(getGuiServicePluginId());
        if (ok) {
