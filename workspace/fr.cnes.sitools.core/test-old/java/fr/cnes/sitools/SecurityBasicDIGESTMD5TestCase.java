...
public class SecurityBasicDIGESTMD5TestCase extends AbstractSitoolsServerTestCase {
  ...
  public static void before() {
    ...
    SitoolsSettings.getInstance().setAuthenticationALGORITHM("DIGEST-MD5");
    start();
  ...
  public void testServerAuthentificationNotEncryptedLogin() {
    ...
    String password = "admin"; // DigestUtils.toHttpDigest(user, "admin".toCharArray(), "SITOOLS");
    ...
    // Send the first request with sufficient authentication for HTTP_BASIC clear text.
    try {
    ...
    // WADL de la resource
    this.createWadl(url, "security-basic-store-digest");
  }
  ...
  public void testServerAuthentificationBasic() {
    ...
    // RIGHT PASSWORD
    password = "admin"; // DigestUtils.toHttpDigest(user, "admin".toCharArray(), "SITOOLS");
