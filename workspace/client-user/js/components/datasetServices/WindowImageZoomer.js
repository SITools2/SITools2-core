...
sitools.user.component.dataviews.services.WindowImageZoomer = Ext.extend(
		Ext.Window, {
			...
			initComponent : function(config) {
				...
				this.listeners = {
					...
					resize : function(window, width, height) {
						if (!Ext.isEmpty(this.viewer)) {
							var frameElementWidth = width
									- this.getAdditionalWidth();
							var frameElementHeight = height
									- this.getAdditionalHeight();
							this.viewer.setFrameProp([frameElementWidth + "px",
									frameElementHeight + "px"]);
							this.viewer.reset();
				...
				sitools.user.component.dataviews.services.WindowImageZoomer.superclass.initComponent
						.apply(this, arguments);
			},

			/**
			 * Method called when the image is loaded. It is in charge of
			 * resizing the image according to the desktop size
			 */
			onImageLoad : function() {
				var img = this.panel.getEl().child("img");
				this.viewer = new viewer({
							image : img.dom,
							frame : [this.sizeLimitWidth + "px",
									this.sizeLimitHeight + "px"],
							zoomFactor : this.zoomFactor,
							maxZoom : this.maxZoom,
							previewSrc : this.previewSrc
						});
				this
						.updateWindowSize(this.sizeLimitHeight,
								this.sizeLimitWidth)
				this.doLayout();
			},
			...
			getAdditionalHeight : function() {
				...
				if (!Ext.isEmpty(this.viewer)
						&& !Ext.isEmpty(this.viewer.toolbarHeight)) {
			...
			listeners : {
				render : function(c) {
					Ext.QuickTips.register({
								target : c,
								text : i18n
										.get('label.thumbnailColumnImageTooltip')
							});
				}
			},
