...
sitools.user.component.DatasetOverview = function (config) {
    ...
    this.formsContainerPanel = new Ext.Panel({
		...
		collapsible : true, 
		split : true, 
		// customFormWidth existe quand des préférences utilisateur sont sauvegardés
		width : (this.customFormWidth) ? this.customFormWidth : this.DEFAULT_WIDTH_FORMS_PANEL, 
...
Ext.extend(sitools.user.component.DatasetOverview, Ext.Panel, {
    ...
    _getSettings : function () {
        ...
        if (this.formsTabPanel.items.length > 0){
	        globalSettings.formsActivePanel = this.formsTabPanel.items.findIndex('id', this.formsTabPanel.activeTab.id);
	        globalSettings.formsPanelWidth = this.formsTabPanel.getWidth();
	        
	        globalSettings = Ext.apply(globalSettings, this.formsTabPanel.activeTab._getSettings());
        }
        else if (this.dataview) {
            globalSettings = Ext.apply(globalSettings, this.dataview._getSettings());
        }
