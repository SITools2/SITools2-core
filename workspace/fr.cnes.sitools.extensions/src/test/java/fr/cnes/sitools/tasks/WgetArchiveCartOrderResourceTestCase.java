...
import fr.cnes.sitools.common.AbstractSitoolsTestCase;
 ...
 * @author m.gond
 */
public class WgetArchiveCartOrderResourceTestCase extends AbstractTaskResourceTestCase {
  ...
  private String orderResourceModelClassName = "fr.cnes.sitools.resources.order.cart.wget.WgetArchiveOrderResourceModel";
  ...
  private void testOrder(MediaType mediaType, String archiveType) throws ClassNotFoundException,
    ...
    create(resourceModel, getBaseProjectUrl());
    ...
    TaskModel taskModel = invokeOrder(urlAttach, "?archiveType=" + archiveType + "&cartFile=" + cartFile + "&fileName="
        + fileName);
    ...
    assertArchiveType(order, archiveType);
    ...
    // queryResourceHTML();
    delete(resourceModel, getBaseProjectUrl());
   ...
   */
  private void assertArchiveType(Order order, String archiveType) {
    assertNotNull(order);
    ...
    try {
      ...
      if (archiveType.equals("tar"))
        assertEquals(MediaType.APPLICATION_TAR, result.getMediaType());

      if (archiveType.equals("tar.gz"))
        assertEquals(MediaType.APPLICATION_TAR, result.getMediaType());
